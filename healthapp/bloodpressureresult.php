...
// Get cluster metrics
$metrics_sql = "SELECT * FROM bp_clustering_metrics";
$metrics_result = $conn->query($metrics_sql);

// Get cluster assignments
$clusters_sql = "SELECT * FROM bp_clustering_results";
$clusters_result = $conn->query($clusters_sql);

// Get optimal K
$optimal_k_sql = "SELECT k FROM bp_clustering_metrics WHERE optimal_k = 1 LIMIT 1";
...
if ($clusters_result->num_rows > 0) {
    while($row = $clusters_result->fetch_assoc()) {
        ...
        $numeric_stats[$cluster]['systolic_sum'] += $row['systolic'];
        $numeric_stats[$cluster]['diastolic_sum'] += $row['diastolic'];
        
        $current_bp = $row['systolic'] . '/' . $row['diastolic'];
        if ($row['systolic'] < $numeric_stats[$cluster]['bp_min']) {
            $numeric_stats[$cluster]['bp_min'] = $row['systolic'] . '/' . $row['diastolic'];
        }
        if ($row['systolic'] > $numeric_stats[$cluster]['bp_max']) {
            $numeric_stats[$cluster]['bp_max'] = $row['systolic'] . '/' . $row['diastolic'];
        }
        
        // Categorical features
        $categorical_stats[$cluster]['gender'][$row['gender']]++;
        $categorical_stats[$cluster]['membership'][$row['membership']]++;
...
for ($i = 0; $i < $optimal_k; $i++) {
    ...
    $cluster_profiles[$i]['systolic_mean'] = 
        $numeric_stats[$i]['systolic_sum'] / $numeric_stats[$i]['age_count'];
    $cluster_profiles[$i]['diastolic_mean'] = 
        $numeric_stats[$i]['diastolic_sum'] / $numeric_stats[$i]['age_count'];
    
    $cluster_profiles[$i]['bp_min'] = $numeric_stats[$i]['bp_min'];
    $cluster_profiles[$i]['bp_max'] = $numeric_stats[$i]['bp_max'];
...
<head>
    ...
    <title>Blood Pressure Clustering Results</title>
    ...
    <style>
        ...
        .bp-high { color: #dc3545; font-weight: bold; }
        .bp-normal { color: #28a745; }
    </style>
</head>
<body>
    <div class="container mt-4">
        <h1 class="mb-4">Blood Pressure Clustering Results</h1>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <?php foreach ($cluster_profiles as $cluster => $profile): 
                    // Determine BP classification for display
                    $systolic_mean = $profile['systolic_mean'];
                    $diastolic_mean = $profile['diastolic_mean'];
                    $bp_class = ($systolic_mean >= 140 || $diastolic_mean >= 90) ? 'bp-high' : 'bp-normal';
                ?>
                <div class="mb-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered profile-table">
                            ...
                            <tr>
                                ...
                                <td class="<?= $bp_class ?>">
                                    Mean: <?= number_format($systolic_mean, 1) ?>/<?= number_format($diastolic_mean, 1) ?> mmHg<br>
                                    Range: <?= $profile['bp_min'] ?> to <?= $profile['bp_max'] ?> mmHg
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    Member: <?= $profile['membership_Yes'] ?>% | 
                                    Non-member: <?= $profile['membership_No'] ?>%
                                </td>
                            </tr>
                            <tr>
                                <th>Top Puroks</th>
